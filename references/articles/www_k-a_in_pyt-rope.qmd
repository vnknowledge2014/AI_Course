...
RoPE was introduced in the paper "RoFormer: Enhanced Transformer with Rotary Position Embedding" and has become an important technique in modern transformer architectures. The fundamental idea is elegant: encode positional information directly into the token representations by performing rotations in the feature space.
...
For the angle  $ \theta\_i $  used for the  $ i $ -th pair, the paper suggests:

$$$
\theta\_i = 10000^{-\frac{2(i-1)}{d}}, i \in [1, 2, ..., \frac{d}{2}]
$$$
...
* computes the  $ \theta $  values for each feature pair using the formula  $ \theta\_i = 10000^{-\frac{2(i-1)}{d}} $
