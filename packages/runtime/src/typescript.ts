...
/** Trừu tượng hoá Worker để dùng chung cho trình duyệt, Node và WebView. */
...
export class BoThucThiTypeScript implements BoThucThi {
  ...
  /** @param taoWorker Tạo một worker mới. Gọi lại mỗi khi worker cũ bị giết. */
  ...
  async chay(ma: string, tuyChon: TuyChonChay = {}): Promise<KetQuaChay> {
    ...
    const phanHoi = await new Promise<TinNhanTuWorker | null>((resolve) => {
      const dongHo = setTimeout(() => {
        // Giết worker: đây là cách DUY NHẤT ngắt được vòng lặp vô hạn trong JS.
        cong.giet();
        this.cong = null;
        resolve(null);
      }, hetHan);
...
export function chanDoanQuaGio(hetHanMs: number): ChanDoan {
  return {
    ma: 'TS0500',
    muc: 'loi',
    thongDiep: `chương trình chạy quá ${hetHanMs / 1000} giây nên đã bị dừng`,
    dong: 1,
    cot: 1,
    doDai: 1,
    viSao:
    ...
    cachSua: [
      'kiểm tra xem biến trong điều kiện vòng lặp có thực sự thay đổi không, ví dụ `i++`',
      'với `while (true)`, chắc chắn có một nhánh `break` chạy tới được',
    ],
    khaiNiem: 'vòng lặp',
    vanBan: `lỗi [TS0500]: chương trình chạy quá ${hetHanMs / 1000} giây nên đã bị dừng`,
  };
}
 ...
 *
 * Thông báo gốc của V8 viết cho lập trình viên đã biết JS. `undefined is not a
 * function` không nói cho người mới biết phải làm gì.
 */
export function chanDoanTuLoiJs(loi: string): ChanDoan {
  ...
  if (ten === 'ReferenceError') {
    ...
    ma = 'TS0502';
    viSao = 'bạn đang gọi một thứ không phải hàm — thường là do gõ sai tên phương thức';
    ...
    ma = 'TS0503';
    viSao =
      'giá trị đang là `undefined` hoặc `null`, nên không lấy được thuộc tính bên trong';
